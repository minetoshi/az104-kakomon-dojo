...
類似問題は過去問と同じ論点を別シナリオ・別数値・別の問い方で確認できるようにしています。公式範囲補強157問は、2026年4月17日時点の Microsoft Learn AZ-104 学習ガイドを基準に、最初の90問で公式範囲と深さを補強し、さらに「参考書なしで問題から学ぶ」前提の独立ギャップ監査で見つかった28項目の薄さを、cov-q091〜cov-q145の55問に加え、外部クロスチェックで cov-q146〜cov-q155 の10問を追加補強し、最終再監査で Encryption at host の異なる判断軸を cov-q156〜cov-q157 の2問で補ったものです。

## 過去問道場
`index.html` に357問対応のページを実装しています。

主な機能:
- 全357問 / 過去問100問 / 第1セット / 第2セット / 類似問題100問 / 公式範囲補強157問の切り替え
- 番号順 / ランダム / 間違えた問題のみ
- カテゴリ絞り込み
...
- 正答インデックス異常なし
- 複数選択問題の正答数チェック済み
- 公式範囲補強157問は全選択肢に明示的な個別解説あり
- 公式範囲補強157問は全問に Microsoft Learn リンクあり
...
- `audit/2026-09-21-post-gap-fill-audit.md`
...
## 2026-09-21 最終ギャップ監査（357問）

355問版を独立基準で再点検したところ、Encryption at host の既存2問がほぼ同一判断軸だったため、制約と既存VMへの適用手順を問う2問（cov-q156〜cov-q157）を追加しました。あわせて cov-q146〜cov-q155 の `kind` をサイト仕様どおり `coverage` に統一しています。

最終構成は **過去問100 + 類似100 + 公式範囲補強157 = 357問**。詳細な構造監査と82スキル再判定は `audit/2026-09-21-post-gap-fill-audit.md` を参照してください。
