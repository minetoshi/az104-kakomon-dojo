...
黒本11章への割り当ては、実際に `index.html` が読み込む366問を `bookChapterOf(q)` で再集計した値です。

| 章 | タイトル | 件数 |
|---:|---|---:|
| 1 | Azureの概要と管理ツール | 27 |
| 2 | Azure Active Directory | 45 |
| 3 | ガバナンスとコンプライアンス | 17 |
| 4 | 仮想マシン | 24 |
| 5 | ストレージ | 67 |
| 6 | 仮想ネットワーク | 22 |
| 7 | サイト間接続 | 19 |
| 8 | ネットワークトラフィック管理 | 33 |
| 9 | App Serviceとコンテナー | 59 |
| 10 | データ保護 | 36 |
| 11 | 監視 | 17 |
| **合計** |  | **366** |

おすすめ学習ルート6分野も同じ366問を重複なく分類し、**61 / 68 / 55 / 56 / 71 / 55問 = 366問** です。

現行件数は `inventory.js` を中央台帳とし、`index.html` 起動時に総数・セット別・章別・おすすめ分野別に加えて、**5セット × 11章 = 55セル**を実データと照合します。不一致があればページ上にエラーを表示します。詳細は `audit/2026-09-22-count-consistency-audit.md` を参照してください。
...
- 番号順 / ランダム / 間違えた問題のみ
- カテゴリ絞り込み
- 10問 / 20問 / 30問 / 50問 / 100問 / 200問 / 366問 / 対象すべて
- 選択肢シャッフル
- 未回答のみ
- 単一選択 / 複数選択
- 正誤判定、正答、解説
...
- `audit/2026-09-21-post-gap-fill-audit.md` ← 当時360問版の最終監査
...
## 2026-09-22 勉強モード / 模試モード分離

トップ画面を **「勉強する」** と **「模試を受ける」** の2つに分離しました。

### 勉強する
- 全366問が対象
- 過去問100 + 類似100 + 公式範囲補強160 + Learn製品補強6
- 公式82スキル細目と、AZ-104 Microsoft Learn に登場する関連製品まで学習対象
- 初期設定はランダム出題 + 選択肢シャッフル
- 解答前はカテゴリ / 黒本章を隠し、サービス名のヒントを与えない
- 解答後にカテゴリ / 章、正解理由、全選択肢解説、Microsoft Learn リンクを表示
- Learn製品補強6問も、単純な製品名当てから本番寄りのシナリオ問題へ改訂

### 模試を受ける
- 50問 / 100分
- 過去問 + 公式範囲補強から公式5分野の比率で抽選
- 類似問題(set 3) と Learn製品補強(set 5) は除外
- 解答中は正誤・解説・カテゴリ / 章を非表示
- 終了後に総合・分野別成績を表示

途中保存は既存の localStorage キーを維持し、トップ画面から「続きから」で学習 / 模試のどちらも再開できます。


## 2026-09-22 勉強モード難易度アップ

勉強モードを「用語暗記」ではなく、要件を読んで判断する本番寄りの教材にするため、こちらで作成した問題を再監査しました。

- Learn製品補強6問を全面的にシナリオ型へ改訂
- 公式範囲補強から簡単すぎる37問を追加で難化
- 合計43問を今回の難易度改善対象として更新
- ランダム出題 + 選択肢シャッフルを勉強モードの初期値に設定
- 解答前はカテゴリ / 黒本章を非表示
- 166補強問を再スキャンし、「短い名称当て型」の高リスク問題は0件
- 構文 / UID / 正答 index / multi / 全選択肢解説の構造異常は0件

詳細:
- `audit/2026-09-22-study-difficulty-upgrade.md`


## 2026-09-22 おすすめ学習ルート

勉強モード内に、推奨順でそのまま進められる「おすすめ学習ルート」を追加しました。
...
各ステップ:
- 過去問 / 類似 / 公式補強 / Learn製品を含む全366問から、該当分野を自動抽出
- 未回答問題を優先して1回20問
- 全問を一度解き、通算正答率80%以上で「完了」
- 現在おすすめのステップに「次にやる」を表示
- 完了後も20問単位で復習可能
- 6分野完了後に全366問から30問の「全範囲ミックス」を解放
...
## 2026-09-25 勉強モードのマスター対象を明確化

勉強モードの最終ゴールを **366 / 366問マスター** に統一しました。

マスター = **同じ問題を2回連続で正解**。

学習順:
1. **中核260問** = 第1セット50 + 第2セット50 + 公式範囲補強160
2. **応用・類似100問** = 類似問題100
3. **Learn補完6問** = Learning Path 周辺製品補強
4. **366 / 366問** で完全マスター
...
- ストレージ 49
- VM・Compute 42
- App Service・コンテナー 40
- ネットワーク 46
- Monitor・Backup・DR 44

②は①完了後、③は①+②完了後におすすめ導線上で解放します。自由学習から任意セットを選ぶ機能は維持しています。

`inventory.js` に mastery 件数と6分野内訳を台帳化し、`scripts/validate-data.js` と `index.html` 起動時監査で実データと照合します。

詳細:
- `audit/2026-09-25-mastery-plan.md`